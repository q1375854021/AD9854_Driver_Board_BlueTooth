...
{   //为了配置PA0~PA7而不改变其它引脚状态，应当把BSRR的第16~23位置1，从而清零低八位，然后在把第八位重新配置一下就可以了
	AD9854_DataBus |= 0x00FF0000;               //清除PA0~PA7
	AD9854_DataBus |= dat;
	AD9854_AdrBus |= 0x01F80000;                       //清除PB3~PB8    //0000 0001 1111 1000
	AD9854_AdrBus |= addr << 3;                         //低6位  0~5 -> 3~8
	AD9854_WR      = 0;                         //然后WR上给一个脉冲，传送数据到AD9854的Buffer
	AD9854_WR      = 1;
}
...
void AD9854_Init(void)
{
	...
	AD9854_WR_Byte(0x1d,0x00);					//打开比较器供电开关，QDAC供电开关，DAC供电开关，数字供电开关。
	//	AD9854_WR_Byte(0x1d,0x10);	    //关闭比较器
	AD9854_WR_Byte(0x1e,CLK_Set);	   		//设置系统时钟倍频        0C  0000_1100      倍频01100，就是12倍倍频，240MHz，不过已经超过200MHz了，PLL range bit应当为1，也就是应当设置为0100_1100 应当是4C
	AD9854_WR_Byte(0x1f,0x00);	   			//设置系统为模式0，由外部更新     ACC1 AC2 Triangle SRC_QADC都没有用到
	AD9854_WR_Byte(0x20,0x60);	   			//设置为可调节幅度，取消插值补偿     //0110_0000   内部逆sinc滤波器旁路掉了，然后OSK EN设为一，可以控制幅度  最后其实可以做一下逆sinc滤波的
...
void AD9854_SetSine(ulong Freq,uint Shape)
{
	...
	{
		AD9854_WR_Byte(Adress++,FreqWord[--count]);   //写入频率FTW1
	}
...
//入口参数:无
//出口参数:无
// 仅仅是初始化了FSK而已，还没有配置FSK的FTW1和FTW2。
//====================================================================================
...
void AD9854_InitOSK(void)
{
	...
	AD9854_WR_Byte(0x20,0x70);	           //设置为可调节幅度，取消插值补偿,通断整形内部控制    0111_0000   OSK斜率内部整形，如果设置为0110_0000，那么就是用I路通道和Q路通道来控制了  。。不过设置成AM模式了
...
//入口参数:无
//出口参数:无
//斜率FSK模式，还开启了Triange，就是双斜率chirp，频率由低变高，再由高变低，只是这里和FSK引脚没有什么关系了
//====================================================================================
void AD9854_InitRFSK(void)
{
	...
	AD9854_WR_Byte(0x1d,0x00);	       //打开比较器
	AD9854_WR_Byte(0x1e,CLK_Set);	   //设置系统时钟倍频
	AD9854_WR_Byte(0x1f,0x24);	        //设置系统为模式2，由外部更新,使能三角波扫频功能   0010_0100  Triangle使能，010模式。
	AD9854_WR_Byte(0x20,0x60);	        //设置为可调节幅度，取消插值补偿	
...
//         频率 上升或者下降 一个步进频率
//   比如系统时钟300MHz，那么一秒输出300M个Sample，如果FreRate是1，一秒输出仍然是300M个Sample，如果是2，那么只输出150M个Sample
//====================================================================================
...
uint8_t AD9854_Uartdata_Analysis(struct Uart_Data_Struc *uart_data_struc)
{
	...
	switch( uart_data_struc->cmd)       //根据cmd决定吧数据给谁赋值
	{
		// 命令1   单频率输出
		case (0x01):
			for(temp_i=0;temp_i<4;temp_i++)          //数据1 4个byte  频率1  1-150MHz   48bit
			{
				uart_data_struc->Data1.Freq1 = ( (uart_data_struc->Data1.Freq1)| USART1_Data_Recv[current_index]<<8*(4-1-temp_i));
				current_index++;
			}
			for(temp_i=0;temp_i<2;temp_i++)         //数据2 2个byte  幅度 1-4095    12bit
			{
		...
		// 命令3   BPSK输出
		case 0x03:
			for(temp_i=0;temp_i<2;temp_i++)          //数据1 2个byte   相位1   1-16383  14bit
			{
				uart_data_struc->Data1.Phase1 = ( (uart_data_struc->Data1.Phase1)| USART1_Data_Recv[current_index]<<8*(2-1-temp_i));
				current_index++;
			}
			for(temp_i=0;temp_i<2;temp_i++)         //数据2 2个byte      相位2   1-16383   14bit
			{
		...
		// 命令4   OSK输出
		case 0x04:
			for(temp_i=0;temp_i<1;temp_i++)          //数据1 1个byte   OSK斜率  0-255  8bit
			{
		...
		// 命令5   AM输出
		case 0x05:
			for(temp_i=0;temp_i<2;temp_i++)          //数据1 2个byte    幅度   1-4095  12bit
			{
